...
@section('title', 'Create Pipeline Template - Recruiter Dashboard')
@section('header_title', 'Create Pipeline Template')

@section('content')
<form action="{{ route('pipelines.create') }}" method="POST" class="space-y-6" x-data="pipelineFormHandler()">
    @csrf
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="col-span-1 bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-6 h-fit sticky top-24">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" class="form-input text-xs" placeholder="e.g. Technology Pipeline" required>
            </div>

            <!-- Description -->
            <div>
                <label for="description" class="form-label">Description</label>
                <textarea name="description" id="description" rows="3" class="form-input text-xs" placeholder="Describe this hiring workflow..."></textarea>
            </div>

            <!-- Hint -->
            <div>
                <label for="department_hint" class="form-label">Department Hint (Optional)</label>
                <input type="text" name="department_hint" id="department_hint" class="form-input text-xs" placeholder="e.g. Technology, Customer Care">
            </div>
                    ...
                    <div class="p-4 bg-[#F7F8FA] border border-gray-200 rounded-xl space-y-4 relative animate-fade-in">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-white p-3 rounded-xl border border-gray-150/60">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" :name="`stages[${index}][auto_notify_candidate]`" x-model="stage.auto_notify_candidate" value="1" class="sr-only peer">
                            ...
                            <div class="flex items-center justify-between border-t md:border-t-0 md:border-l border-gray-100 pt-2 md:pt-0 md:pl-4">
                                ...
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" :name="`stages[${index}][is_terminal_pass]`" x-model="stage.is_terminal_pass" @change="onTerminalPassChange(index)" value="1" class="sr-only peer">
                                    <div class="w-7 h-4 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:bg-[#FF6B00]"></div>
                                </label>
                            </div>
                            ...
                            <div class="flex items-center justify-between border-t md:border-t-0 md:border-l border-gray-100 pt-2 md:pt-0 md:pl-4">
                                ...
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" :name="`stages[${index}][is_terminal_fail]`" x-model="stage.is_terminal_fail" @change="onTerminalFailChange(index)" value="1" class="sr-only peer">
                                    <div class="w-7 h-4 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:bg-[#FF6B00]"></div>
                                </label>
                            </div>
                        </div>
...
<script>
    function pipelineFormHandler() {
        return {
            stages: [
                { name: 'Applied', description: 'Application submitted', color: '#FF6B00', auto_notify_candidate: true, is_terminal_pass: false, is_terminal_fail: false },
                { name: 'Screening', description: 'Candidate screening checklist', color: '#F59E0B', auto_notify_candidate: true, is_terminal_pass: false, is_terminal_fail: false },
                { name: 'Hired', description: 'Candidate successfully onboarded', color: '#10B981', auto_notify_candidate: true, is_terminal_pass: true, is_terminal_fail: false },
                { name: 'Rejected', description: 'Hiring process closed', color: '#EF4444', auto_notify_candidate: true, is_terminal_pass: false, is_terminal_fail: true }
            ],
